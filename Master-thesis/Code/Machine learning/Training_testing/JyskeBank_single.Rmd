...
```{r}
data_JyskeBank <- data %>% select(Date, JyskeBank)
```
...
```{r}
training <- data_JyskeBank %>% filter(Date <= "2017-12-29")
testing <- data_JyskeBank %>% filter(Date > "2017-12-29" & Date <= "2018-12-28")
```
...
tuner <- Hyperband(
  ...
  project_name = "JyskeBank_hyper_FNN_single")
```
...
tuner <- Hyperband(
  ...
  max_epochs = 10,
  hyperband_iterations = 5,
  project_name = "JyskeBank_hyper_RNN_single")
```
...
tuner <- Hyperband(
  ...
  max_epochs = 10,
  hyperband_iterations = 5,
  project_name = "JyskeBank_hyper_LSTM_single")
```
...
tuner <- Hyperband(
  ...
  max_epochs = 10,
  hyperband_iterations = 5,
  project_name = "JyskeBank_hyper_biLSTM_single")
```
...
tuner <- Hyperband(
  ...
  max_epochs = 10,
  hyperband_iterations = 5,
  project_name = "JyskeBank_hyper_GRU_single")
```
...
tuner <- Hyperband(
  ...
  max_epochs = 10,
  hyperband_iterations = 5,
  project_name = "JyskeBank_hyper_biGRU_single")
```
...
```{r}
ANN_prediction_unscale <- ANN_prediction * att_JyskeBank
...
y_test_unscale <- y_test * att_JyskeBank
```
...
```{r}
save_model_hdf5(model_ANN, "model_ANN_JyskeBank_single")
save_model_hdf5(model_RNN, "model_RNN_JyskeBank_single")
save_model_hdf5(model_LSTM, "model_LSTM_JyskeBank_single")
save_model_hdf5(model_biLSTM, "model_biLSTM_JyskeBank_single")
save_model_hdf5(model_GRU, "model_GRU_JyskeBank_single")
save_model_hdf5(model_biGRU, "model_biGRU_JyskeBank_single")
Results %>% select(Model, RMSE, MAPE) %>% write.csv("Results_JyskeBank_single")
```