...
```{r}
data <- read_csv("https://raw.githubusercontent.com/andreasbj77/Master-thesis/main/Data/Final_data/DataCreationVestas.csv")
```
...
```{r}
data$Vestas %<>% scale(center = FALSE, scale = TRUE)
data$Genmab %<>% scale(center = FALSE, scale = TRUE)
data$NovoNordisk %<>% scale(center = FALSE, scale = TRUE)
data$USD %<>% scale(center = FALSE, scale = TRUE)
data$NOK %<>% scale(center = FALSE, scale = TRUE)
data$C20index %<>% scale(center = FALSE, scale = TRUE)
data$SP500 %<>% scale(center = FALSE, scale = TRUE)

att_Vestas <- attr(data$Vestas, 'scaled:scale')

data$Vestas %<>% as.numeric()
data$Genmab %<>% as.numeric()
data$NovoNordisk %<>% as.numeric()
data$USD %<>% as.numeric()
data$NOK %<>% as.numeric()
data$C20index %<>% as.numeric()
...
The variables needed to forecast Vestas is selected from the complete data set
```{r}
data_Vestas <- data
```
...
```{r}
training <- data_Vestas %>% filter(Date <= "2017-12-29")
testing <- data_Vestas %>% filter(Date > "2017-12-29" & Date <= "2018-12-28")
```


The data sets are divided into the response and explanatory variables. Here a lead of Vestas acts as the response variable.
...
y_train <- training %>%
  select(-Date) %>%
  mutate(Vestas = Vestas %>% lead(1)) %>%
  mutate(Vestas = ifelse(is.na(Vestas), lag(Vestas, 1), Vestas)) %>%
  select(Vestas) %>% 
  as.matrix()
...
n_features = ncol(data_Vestas) - 1 # amount of explanatory variables
...
tuner <- Hyperband(
  ...
  max_epochs = 10,
  hyperband_iterations = 5,
  project_name = "Vestas_hyper_LSTM")
```
...
tuner <- Hyperband(
  ...
  max_epochs = 10,
  hyperband_iterations = 5,
  project_name = "Vestas_hyper_biLSTM")
```
...
tuner <- Hyperband(
  ...
  max_epochs = 10,
  hyperband_iterations = 5,
  project_name = "Vestas_hyper_biGRU")
```
...
```{r}
ANN_prediction_unscale <- ANN_prediction * att_Vestas
RNN_prediction_unscale <- RNN_prediction * att_Vestas
LSTM_prediction_unscale <- LSTM_prediction * att_Vestas
biLSTM_prediction_unscale <- biLSTM_prediction * att_Vestas
GRU_prediction_unscale <- GRU_prediction * att_Vestas
biGRU_prediction_unscale <- biGRU_prediction * att_Vestas

y_test_unscale <- y_test * att_Vestas
```
...
```{r}
save_model_hdf5(model_ANN, "model_ANN_Vestas")
save_model_hdf5(model_RNN, "model_RNN_Vestas")
save_model_hdf5(model_LSTM, "model_LSTM_Vestas")
save_model_hdf5(model_biLSTM, "model_biLSTM_Vestas")
save_model_hdf5(model_GRU, "model_GRU_Vestas")
save_model_hdf5(model_biGRU, "model_biGRU_Vestas")
Results %>% select(Model, RMSE, MAPE) %>% write.csv("Results_Vestas")
```
